...
pub enum AppEvent {
    ...
    /// Open a server-local project path in the graphical file manager. A headless
    /// server forwards this host-local side effect to its foreground local client.
    OpenInFileManager { path: std::path::PathBuf },
