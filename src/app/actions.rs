...
impl AppState {
    /// Resolves the openable target at a pane cell: a web URL, an OSC 8 target,
    /// or an existing local path rendered as a `file://` URL.
    pub(crate) fn open_target_at_pane_surface_cell(
        &self,
        ...
        let cwd = self.pane_cwd_in_workspace(ws_idx, pane_id, terminal_runtimes);
        open_target_at_runtime_cell(
            rt,
            pane_id,
    ...
    /// The working directory a pane's relative links resolve against.
    fn pane_cwd_in_workspace(
        &self,
        ws_idx: usize,
        pane_id: crate::layout::PaneId,
        terminal_runtimes: &crate::terminal::TerminalRuntimeRegistry,
        ...
        workspace
            .tabs
            .iter()
            .find_map(|tab| tab.cwd_for_pane(pane_id, &self.terminals, terminal_runtimes))
            .or_else(|| Some(workspace.identity_cwd.clone()))
    }
}

fn open_target_at_runtime_cell(
    ...
    {
        return Some(resolved_hyperlink_target(&uri, cwd).unwrap_or(uri));
    }
    ...
    path_at_column(line, logical_cell.logical_col, cwd).map(|path| local_path_file_url(&path))
}

/// Keeps web hyperlinks as-is and renders a hyperlink that points at an
/// existing local file as a `file://` URL, matching visible-text behavior.
fn resolved_hyperlink_target(uri: &str, cwd: Option<&std::path::Path>) -> Option<String> {
    if safe_web_url(uri).is_some() {
        return None;
    }
    if uri.starts_with("file://") {
        return existing_local_path(uri, cwd).map(|path| local_path_file_url(&path));
    }
    None
}
...
/// Whether a resolved pane target is safe to hand to the host opener.
pub(crate) fn safe_open_target(target: &str) -> Option<&str> {
    if safe_web_url(target).is_some() || target.starts_with("file://") {
        Some(target)
    } else {
        None
    }
...
/// Decodes a `file://` URL that the pane resolver produced back into a path.
pub(crate) fn local_path_from_file_url(target: &str) -> Option<PathBuf> {
    target
        .starts_with("file://")
        .then(|| local_path_candidate(target))
        .flatten()
}
...
/// Resolves the visible text at `col` to an existing local file or directory.
/// Relative candidates resolve against `cwd`, and a trailing `:line[:col]`
/// location suffix is retried without the suffix.
fn path_at_column(row: &str, col: u16, cwd: Option<&std::path::Path>) -> Option<PathBuf> {
    let cells = text_cells(row);
    let clicked_idx = cell_index_at_column(&cells, col)?;
...
fn existing_local_path(candidate: &str, cwd: Option<&std::path::Path>) -> Option<PathBuf> {
    ...
    for _ in 0..=2 {
        ...
        // Editors and this terminal print `path:line:column`; retry the path alone.
...
fn local_path_candidate(candidate: &str) -> Option<PathBuf> {
    ...
    // A Windows drive path arrives as `/D:/dir`; drop the added leading slash.
    let native_path = decoded_path
        .strip_prefix('/')
        .filter(|without_leading_slash| std::path::Path::new(without_leading_slash).is_absolute())
        .unwrap_or(&decoded_path);
    Some(PathBuf::from(native_path))
}

/// Renders a local path as a `file://` URL for the client-side opener.
fn local_path_file_url(path: &std::path::Path) -> String {
    let display = crate::platform::path_for_user_display(path).replace('\\', "/");
    let mut url = String::from("file:///");
    if let Some(without_root) = display.strip_prefix('/') {
        url.push_str(without_root);
    } else if display.starts_with("//") {
        // UNC share: `//server/share` keeps both slashes after the empty authority.
        url.push_str(&display[1..]);
    } else {
...
fn percent_encode_url_path(value: &str) -> String {
    ...
    for byte in value.bytes() {
        ...
        if unreserved || matches!(byte, b'/' | b':') {
...
mod tests {
    ...
    #[test]
    fn path_at_column_resolves_existing_files_directories_and_source_locations() {
        let root = std::env::temp_dir().join(format!(
            "herdr-path-at-column-{}-{}",
            std::process::id(),
        ...
        assert_eq!(
            path_at_column(&absolute, 3, None).as_deref(),
            Some(file.as_path())
        );
        // Editors print `path:line:column`; the suffix is stripped before lookup.
        let located = format!("{absolute}:12:4");
        assert_eq!(
            path_at_column(&located, 3, None).as_deref(),
            Some(file.as_path())
        );
        // A relative candidate resolves against the pane's working directory.
        assert_eq!(
            path_at_column("src/main.rs", 2, Some(root.as_path())).as_deref(),
            Some(file.as_path())
        );
        assert_eq!(
            path_at_column("src", 1, Some(root.as_path())).as_deref(),
            Some(root.join("src").as_path())
        );
        assert!(path_at_column("missing.rs", 1, Some(root.as_path())).is_none());

        let _ = std::fs::remove_dir_all(&root);
    }

    #[test]
    fn local_path_file_url_round_trips_through_the_client_decoder() {
        let path = std::path::Path::new("/repo dir/src/main.rs");
        let url = local_path_file_url(path);

        assert!(url.starts_with("file:///"), "{url}");
        assert!(url.contains("%20"), "spaces must be encoded: {url}");
        #[cfg(not(windows))]
        assert_eq!(local_path_from_file_url(&url).as_deref(), Some(path));
    }
