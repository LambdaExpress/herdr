...
fn ctrl_click_routes_link_activation_through_endpoint_then_client_host() {
    ...
    assert!(matches!(
        &actions[..],
        [ClientShellAction::OpenTarget(url)] if url == "https://example.test"
    ));
...
#[test]
fn alt_drag_selects_a_block_without_forwarding_the_gesture() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    let mut pane_surface = surface();
    pane_surface.panes[0].mouse_reporting = true;
    // The default fixture pane is 4x2, which cannot express a rectangular drag.
    let pane_fixture = &mut pane_surface.panes[0];
    pane_fixture.rect.width = 20;
    pane_fixture.rect.height = 8;
    pane_fixture.inner_rect.width = 20;
    pane_fixture.inner_rect.height = 8;
    ...
    let started = state.handle_raw_events(vec![RawInputEvent::Mouse(down)]);

    assert!(
        started.requests.is_empty(),
        "a block gesture never reaches the pane application"
    );
    let selection = state.selection.as_ref().expect("block selection");
    ...
    assert_eq!((end_row - start_row, end_col - start_col), (2, 3));
    assert!(selection.contains(2, 4, pane.scroll));
    assert!(
        !selection.contains(2, 0, pane.scroll),
        "a block selection holds its column bounds on every row"
    );
}
