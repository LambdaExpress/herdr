...
#[test]
fn workspace_menu_copies_the_project_path_and_hides_unavailable_file_manager_actions() {
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot()));
    state.set_pane_surface(surface());
    state.compose(106, 20).expect("composed frame");
    ...
    let copy_index = items
        .iter()
        .position(|item| item.action == ClientContextMenuAction::CopyProjectPath)
        .expect("project path copy entry");
    let open_index = items
        .iter()
        .position(|item| item.action == ClientContextMenuAction::OpenInFileManager);

    // The file-manager entry exists exactly when the host can show one.
    assert_eq!(
        open_index.is_some(),
        crate::platform::file_manager_available()
    );

    let mut outcome = ClientShellInput::default();
    state.activate_context_menu_item(copy_index, &mut outcome);
    let expected = crate::platform::path_for_user_display(std::path::Path::new("/repo"));
    assert!(matches!(
        &outcome.actions[..],
        [ClientShellAction::ClipboardWrite(bytes)] if bytes.as_slice() == expected.as_bytes()
    ));

    if let Some(open_index) = open_index {
        // Activating an entry closes the menu, so reopen it for the second action.
        state.open_workspace_context_menu("ws_1".into(), 0, 0);
        let mut outcome = ClientShellInput::default();
        state.activate_context_menu_item(open_index, &mut outcome);
        assert!(matches!(
            &outcome.actions[..],
            [ClientShellAction::OpenInFileManager(path)] if path == "/repo"
        ));
    }
}
