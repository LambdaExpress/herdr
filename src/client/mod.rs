...
fn run_client_with_mode(
    ...
    let host_graphics_protocol =
        crate::kitty_graphics::sixel::host_graphics_protocol(kitty_graphics_enabled);
    // A SIXEL host is sized by the encoder's virtual grid, so its real pixel
    // geometry is never queried.
    let pixel_geometry_enabled =
        (kitty_graphics_enabled && !host_graphics_protocol.is_sixel()) || attach_escape.is_some();
    ...
    let loop_config = ClientLoopConfig {
        ...
        pixel_geometry_fallback: kitty_graphics_enabled && !host_graphics_protocol.is_sixel(),
    ...
    // Report the grid the host encoder draws against, not the host's own pixels.
    let reported_cell_size =
        host_graphics_protocol.cell_size(crate::kitty_graphics::HostCellSize {
            width_px: cell_width_px,
            height_px: cell_height_px,
        });
    let (cell_width_px, cell_height_px) =
        (reported_cell_size.width_px, reported_cell_size.height_px);
    let exact_cell_size = exact_cell_size && !host_graphics_protocol.is_sixel();
...
async fn run_client_loop(
    ...
    if let Some(shell) = state.shell.as_mut() {
        shell.set_host_graphics_protocol(config.host_graphics_protocol);
    ...
    let will_query_host_cell_size = state.attach_escape.is_none()
        && !state.host_graphics_protocol.is_sixel()
        ...
        if let Some(shell) = state.shell.as_ref() {
            supervisors.spawn_due(
                ...
                endpoint::EndpointConnectOptions {
                    ...
                    cell_width_px: state.graphics_cell_size().0,
                    cell_height_px: state.graphics_cell_size().1,
        ...
        match event {
            ...
            ClientLoopEvent::Resize(
                ...
                state.pixel_geometry_exact =
                    pixel_geometry_exact && !state.host_graphics_protocol.is_sixel();
                let (cell_width_px, cell_height_px) = state.graphics_cell_size();
