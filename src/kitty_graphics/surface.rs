...
pub(crate) struct ClientState {
    ...
    sixel: super::sixel::SixelGraphicsCache,
    protocol: super::sixel::HostGraphicsProtocol,
...
impl ClientState {
    pub(crate) fn set_protocol(&mut self, protocol: super::sixel::HostGraphicsProtocol) {
    ...
    pub(crate) fn encode(
        ...
        let mut placements = self
            ...
            .collect::<Vec<_>>();
        if self.protocol.is_sixel() {
            return super::sixel::encode_sixel_placements(
                &mut placements,
                safe_bottom,
                &mut self.sixel,
            );
...
mod tests {
    ...
    fn client_hides_and_restores_without_reuploading_pixels() {
        ...
        let _ = state.encode(Visibility::Main, (4, 2), None, cell, None);

        let hidden = state.encode(Visibility::Hidden, (4, 2), None, cell, None);
        assert!(String::from_utf8_lossy(&hidden).contains("a=d,d=i"));

        let restored = state.encode(Visibility::Main, (4, 2), None, cell, None);
