...
pub(super) fn dispatch_client_shell_actions(
    ...
    for action in actions {
        match action {
            ...
            shell::ClientShellAction::OpenTarget(target) => {
                if crate::app::actions::safe_open_target(&target).is_some() {
                    let opened = match crate::app::actions::local_path_from_file_url(&target) {
                        Some(path) => crate::platform::open_target(path.as_os_str()),
                        None => crate::platform::open_target(std::ffi::OsStr::new(&target)),
                    };
                    match opened {
                        Ok(Some(child)) => detached_process_children.push(child),
                        Ok(None) => {}
                        Err(err) => {
                            warn!(err = %err, target = %target, "failed to open pane target")
                        }
                    }
                }
            }
            shell::ClientShellAction::OpenInFileManager(path) => {
                match crate::platform::open_in_file_manager(std::path::Path::new(&path)) {
                    Ok(Some(child)) => detached_process_children.push(child),
                    Ok(None) => {}
                    Err(err) => warn!(err = %err, path = %path, "failed to open file manager"),
                }
            }
