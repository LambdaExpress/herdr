...
pub(crate) enum ClientShellAction {
    ...
    /// Open a resolved pane target: a web URL or a `file://` local path.
    OpenTarget(String),
    /// Reveal a local path in the host file manager.
...
pub(super) enum ClientContextMenuTarget {
    Workspace {
        ...
        collapsed: bool,
        /// Workspace project directory, present when a path action can act on it.
...
pub(crate) struct ClientShellState {
    ...
    /// Host graphics protocol used to encode this shell's pane graphics.
...
impl ClientShellState {
    pub(crate) fn new(mut config: ClientShellConfig) -> Self {
        ...
        Self {
            ...
            host_graphics_protocol: crate::kitty_graphics::sixel::HostGraphicsProtocol::default(),
