    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Right) => {
                ...
                // An agent row stands for its pane, so it opens that pane's tab menu.
                let agent_tab_id = self
                    .hits
                    .agents
                    .iter()
                    .find(|(rect, _)| super::contains(*rect, point))
                    .and_then(|(_, pane_id)| self.tab_id_for_pane(pane_id).map(str::to_owned));
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                let pane_hit = self
                    ...
                    .cloned();
                if let Some(hit) = pane_hit {
                    // Alt-drag is reserved for a rectangular block selection, so it
                    // never reaches a pane application that reports the mouse.
                    let block_selection = mouse
                        .modifiers
                        ...
                        && !block_selection
                    {
                                ...
                                .is_some_and(|previous| previous.is_double_click_for(&click))
                        {
                            ...
                            self.selection = Some(if block_selection {
                                crate::selection::Selection::block_anchor(
                                    hit.pane_id.clone(),
                                    viewport_row,
                                    col,
                                    hit.scroll,
                                ...
                                crate::selection::Selection::anchor(
                                    hit.pane_id.clone(),
                                    viewport_row,
                                    col,
                                    hit.scroll,
