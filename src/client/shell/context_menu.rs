...
impl ClientContextMenuOverlay {
    pub(super) fn items(&self) -> Vec<ClientContextMenuItem> {
        ...
        let project_path_items = |path: &Option<String>| {
            if path.is_none() {
                return Vec::new();
            }
            let mut items = vec![item("Copy project path", Action::CopyProjectPath)];
            if crate::platform::file_manager_available() {
                items.push(item("Open in file manager", Action::OpenInFileManager));
            }
            items
        };
        match &self.target {
            ...
            ClientContextMenuTarget::Workspace {
                ...
                items.extend(project_path_items(project_path));
                items
            }
            ClientContextMenuTarget::Workspace {
...
impl ClientShellState {
    pub(super) fn open_workspace_context_menu(&mut self, workspace_id: String, x: u16, y: u16) {
        ...
        self.overlay = Some(ClientShellOverlay::ContextMenu(ClientContextMenuOverlay {
            target: ClientContextMenuTarget::Workspace {
                ...
                collapsed,
                project_path: (!workspace.new_workspace_cwd.is_empty())
    ...
    /// The tab that currently owns `pane_id`.
    pub(super) fn tab_id_for_pane(&self, pane_id: &str) -> Option<&str> {
        self.snapshot
            .as_deref()?
            .panes
            .iter()
    ...
    pub(super) fn activate_context_menu_item(
        ...
        match menu.target {
            ClientContextMenuTarget::Workspace {
                ...
                self.activate_workspace_context_action(workspace_id, project_path, action, outcome)
            }
    ...
    fn activate_workspace_context_action(
        ...
        match action {
            ClientContextMenuAction::CopyProjectPath => {
                if let Some(path) = project_path.as_deref() {
                    let display =
                        crate::platform::path_for_user_display(std::path::Path::new(path));
                    outcome
                        .actions
                        .push(ClientShellAction::ClipboardWrite(display.into_bytes()));
                }
            }
            ClientContextMenuAction::OpenInFileManager => {
                if let Some(path) = project_path {
                    outcome
                        .actions
                        .push(ClientShellAction::OpenInFileManager(path));
                }
            }
