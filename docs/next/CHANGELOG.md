...
- Right-clicking an agent sidebar entry now opens that tab's context menu.
- Holding Alt while dragging in a terminal pane now selects and copies a rectangular block of text.
- Ctrl-click now opens existing local files and directories, including `file://` links, in addition to web URLs.

### Fixed
- SIXEL downscaling now preserves thin text strokes and transparent edges instead of dropping source pixels, unchanged payloads are reused across frames, and the host terminal's bottom row is left unused so an image cannot scroll the surface.
...
### Changed
- New tabs now open immediately with generated numeric labels by default. Set `ui.prompt_new_tab_name = true` to restore the naming prompt.
...
### Fixed
- Ctrl-clicking an existing file, directory, or `file://` link in a terminal pane now opens it with the operating system's default handler, resolving relative paths from that pane's working directory.
...
- Windows Terminal image scrolling now reuses unchanged SIXEL payloads and bounds ConPTY writes, preventing multi-image panes from stalling during scroll.
