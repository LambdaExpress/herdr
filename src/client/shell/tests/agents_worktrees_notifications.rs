...
#[test]
fn right_clicking_an_agent_row_opens_its_tab_context_menu() {
    ...
    let Some(ClientShellOverlay::ContextMenu(menu)) = &state.overlay else {
        panic!("agent row right-click should open a context menu");
    };
    let ClientContextMenuTarget::Tab { tab_id, .. } = &menu.target else {
        panic!("agent row right-click should open its tab menu");
    };
