...
pub(super) struct ClientState {
    ...
    /// Host graphics protocol this client emits.
    pub(super) host_graphics_protocol: crate::kitty_graphics::sixel::HostGraphicsProtocol,
...
impl ClientState {
    ...
    pub(super) fn present_frame(&mut self, frame_data: FrameData) {
        ...
        // A SIXEL image outlives the frame that drew it and the next text diff
        // cannot erase it, so keep repainting while one is on screen.
        let keep_repainting = self.host_graphics_protocol.is_sixel() && !graphics.is_empty();
        let _ = write_encoded_frame_with_graphics(
            &mut stdout,
            &encoded.bytes,
            graphics,
        ...
        self.repaint_pending = keep_repainting;
    }

    /// Cell geometry to report to the endpoint.
    ///
    /// SIXEL hosts cannot report real geometry, so they report the fixed
    /// virtual grid the encoder sizes its rasters against.
    pub(super) fn graphics_cell_size(&self) -> (u32, u32) {
        let reported = crate::kitty_graphics::HostCellSize {
            width_px: self.reported_cell_size.0,
            height_px: self.reported_cell_size.1,
        };
        let size = self.host_graphics_protocol.cell_size(reported);
